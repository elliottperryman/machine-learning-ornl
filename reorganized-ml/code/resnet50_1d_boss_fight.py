...
cb = keras.callbacks.ModelCheckpoint("weights-boss-{epoch:02d}-{acc:.2f}.hdf5", monitor='acc', verbose=0, save_best_only=False, save_weights_only=True, mode='auto', period=100)
model = ResNet50(input_shape = (400, 1), classes = 2)
model.compile(optimizer='adamax', loss='binary_crossentropy', metrics=['accuracy'])
model.load_weights("weights-5k-5000-0.99.hdf5")
gen = DataReader(numBeforeRepeated=1, batch_size=32, edgeCases=False)
model.fit_generator(gen, epochs = 1000, callbacks=[cb])
model.save('boss.h5')
